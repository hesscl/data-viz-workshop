...
ggplot(dd_2_period, aes(x = period, y = mean_Y, shape = factor(treat), 
  ...
  geom_point(aes(x = period, y = cf), size = 3) +
  geom_point(size = 3) +
...
ggplot(means, aes(x = year, y = lemp, color = factor(first.treat), group = factor(first.treat))) +
  facet_grid(~ first.treat) +
...
## Hands-on: Use the `injury` data or your own data to make graphics for trends over time.
